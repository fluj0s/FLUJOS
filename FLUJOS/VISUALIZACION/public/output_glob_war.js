...
class Graph {
  ...
  async loadNodes(subtematica = '', palabraClave = '', fechaInicio = '', fechaFin = '') {
    const query = {
      bool: {
        must: [
          { match: { tema: 'guerras y gobiernos autoritarios' } }
        ],
        filter: []
      }
    };
  ...
  async drawGraph(relacionesPath, subtematica = '', palabraClave = '', fechaInicio = '', fechaFin = '') {
    ...
    this.graph
      ...
      .nodeVal(5)
      .linkColor(() => 'red') // Cambiar el color de los enlaces a rojo
      .onNodeClick(node => {
...
document.getElementById('paramForm').addEventListener('submit', function (event) {
  ...
  // Configurar el número máximo de nodos
  myWarGraph.setMaxNodes(maxNodes);
  ...
  myWarGraph.setMinSimilarity(minSimilarity);

  // Dibujar el gráfico con los nuevos parámetros
  myWarGraph.drawGraph('/ruta/al/directorio/de/relaciones', subtematica, palabraClave, fechaInicio, fechaFin);
});

// Inicializar el gráfico con los parámetros por defecto
myWarGraph.drawGraph('/ruta/al/directorio/de/relaciones');
